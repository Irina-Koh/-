...
def mask_account_card(type_and_number: str) -> str:
    ...
    for char in lower_type_and_number:  # отдельно записываем номер карты или счета
        if char.isdigit():
            number = number + char
            if (
                len(number) == 20 and "счет" in lower_type_and_number
            ):  # проверяем тип данных и маскируем номер счета или карты
...
def get_date(date: str) -> str:
    ...
    if (
        ...
        and str(date[16]) == ":"
        and str(date[19]) == "."
    ):  # проверяем правильно ли записаны данные
        day = date[8:10]
        month = date[5:7]
        year = date[0:4]
        if (
            ...
            if int(year) % 100 == 0 and int(year) % 400 != 0 and month == "02" and int(day) < 29:
                ...
                return f"{day}.{month}.{year}"
            elif int(year) % 4 == 0 and int(year) % 100 != 0 and month == "02" and int(day) < 30:
                return f"{day}.{month}.{year}"
            elif (
                month == "01"
                or month == "03"
                ...
                return f"{day}.{month}.{year}"
            elif month == "04" or month == "06" or month == "09" or month == "11" and int(day) < 31:
                return f"{day}.{month}.{year}"
            else:
